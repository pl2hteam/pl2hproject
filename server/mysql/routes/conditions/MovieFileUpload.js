...
function FileUpload(props) {
  ...
  const onDrop = (files) => {
    ...
    if (files[0].type == "video/mp4") {
      Axios.post("/api/mysql/conditions/write", formData, config).then(
        (response) => {
      ...
      console.log("####################################");
      Axios.post("/api/mysql/conditions/uploadimage", formData, config).then(
        (response) => {
          console.log(response.data.image);
          console.log(props.refresh);
          if (response.data.success) {
            setImages(response.data.image);
            props.refresh.updateImages(response.data.image);
          } else {
